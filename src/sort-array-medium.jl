...
function merge_vecs_tree(input::Vararg{Vec{N,T}, L}) where {N,T,L}
    if L==2
...
function merge_vecs_tree(input::AbstractArray{T,A}, ::Val{N}, ::Val{L}) where {N,T,A,L}
    ne = size(input, 1)
    if ne==2*N*L
        chunk_1 = ntuple(l->vload(Vec{N, T}, input, 1 + (l-1)*N), L)
        chunk_2 = ntuple(l->vload(Vec{N, T}, input, N*L + 1 + (l-1)*N), L)
        bitonic_merge(sort_small_array(chunk_1), sort_small_array(chunk_2))
    else
        merge_streams(
            merge_vecs_tree((@view input[1:div(ne,2)]), Val(N), Val(L)),
            merge_vecs_tree((@view input[1+div(ne,2):ne]), Val(N), Val(L))
        )
    end
end

function chipsort_medium(input::AbstractArray{T,A}, ::Val{N}, ::Val{L}) where {A,T,N,L}
    output = valloc(T, div(32, sizeof(T)), size(input,1))
    p=1
    for chunk in merge_vecs_tree(input, Val(N), Val(L))
